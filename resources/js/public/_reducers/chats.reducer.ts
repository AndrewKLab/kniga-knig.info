...
export function chats(state = initialState, action) {
    switch (action.type) {
        ...
        case chatsConstants.PUSH_MESSAGE:            
            return {
                ...state,
                get_one_by_id_chats: (state.get_one_by_id_chats && state.get_one_by_id_chats.messages) && (action?.res?.notification?.data?.new_chat_message?.kk_cm_chat_id == state.get_one_by_id_chats.kk_chat_id) ? {...state.get_one_by_id_chats, messages: [...state.get_one_by_id_chats.messages, action.res.notification.data.new_chat_message]}  : state.get_one_by_id_chats,
            };
