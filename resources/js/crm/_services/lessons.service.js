...
//     return handleResponse(response);
// }
async function getAllByCourseId(params) {
    const response = await fetch(`${config.apiUrl}${api_path}getAllByCourseId${params ? paramsObjectToString(params) : ''}`, { ...config.GET, headers: { ...authHeader() } });
    return handleResponse(response);
}
